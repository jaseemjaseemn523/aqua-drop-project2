...
export default function OrderHistoryPage() {
  ...
  if (loading) return <div className="p-10 text-center text-slate-600">Loading order history...</div>;
        ...
        <div className="space-y-4">
            ...
            <div key={order.id} className="bg-white rounded-2xl shadow p-5 border border-slate-200">
              <div className="flex justify-between items-start mb-3">
                <div>
                  ...
                  {userRole === "ADMIN" && (
                    <p className="text-xs text-slate-500 mt-1">Customer Email: {order.customer_email || order.user}</p>
                  )}
                </div>
          ...
          {orders.length === 0 && (
            <div className="text-center py-16 bg-white rounded-2xl shadow-sm border border-slate-200">
              <p className="text-slate-500 font-medium">No order records found.</p>
            </div>
          )}
        </div>
      </div>
